...
class Message(BaseModel):
    ...
    emotion: EmotionResult | None = None  # 情绪分析结果（可选）
...
class ChatSession(BaseModel):
    ...
    ended_at: datetime | None = None  # 结束时间（可选）
    messages: list[Message] = []  # 消息列表
    emotion_summary: dict | None = None  # 情绪摘要（可选）
...
class Alert(BaseModel):
    ...
    resolved_at: datetime | None = None  # 解决时间（可选）
